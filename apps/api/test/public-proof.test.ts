...
describe('SealedBid API - Public Proof Generation & Verification', () => {
  ...
  it('should generate, retrieve, and verify public Merkle proofs', async () => {
    ...
    for (const bidItem of data.bids) {
      ...
      const verifiedTamperedCommitment = verifyMerkleProof(data.merkleRoot, tamperedCommitment, bidItem.merkleProof);
      ...
      if (tamperedProof.length > 0) {
        ...
        const verifiedTamperedProof = verifyMerkleProof(data.merkleRoot, bidItem.commitment, tamperedProof);
    ...
    const apiVerifyRes = await request
    ...
    const apiVerifyResTampered = await request
      .post('/v1/public/verify')
      .send({
        merkleRoot: data.merkleRoot,
        leaf: '0x' + 'f'.repeat(64),
