...
describe("SealedBid API - Gap Fill Endpoints", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      await prisma.org
        .create({ data: { id: orgId, name: "Gap Fill Test Org", type: "ENTERPRISE" } })
        .catch(() => {});
      await prisma.user
        .create({
          data: { id: managerId, orgId, email: "mgr@gapfill.com", role: "PROCUREMENT_MANAGER" },
        })
        .catch(() => {});
      await prisma.user
        .create({ data: { id: vendorId, orgId, email: "vendor@gapfill.com", role: "VENDOR" } })
        .catch(() => {});
      await prisma.user
        .create({ data: { id: auditorId, orgId, email: "auditor@gapfill.com", role: "AUDITOR" } })
        .catch(() => {});
    } catch (e) {
  ...
  it("should reject confirm-upload from a different vendor", async () => {
    ...
    try {
      await prisma.user
        .create({
          data: { id: otherVendorId, orgId, email: "vendor2@gapfill.com", role: "VENDOR" },
        })
        .catch(() => {});
    } catch (e) {}
  ...
  it("should reject vendor list request from wrong org", async () => {
    const otherOrgToken = app.jwt.sign({
      id: managerId,
      orgId: "cother123456789012345678",
