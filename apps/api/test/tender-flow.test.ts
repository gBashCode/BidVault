...
describe('SealedBid API - Complete Tender + Bid Lifecycle', () => {
  ...
  let orgId = 'org_' + Math.random().toString(36).slice(2);
  let managerId = 'mgr_' + Math.random().toString(36).slice(2);
  let vendorId = 'vnd_' + Math.random().toString(36).slice(2);
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      await prisma.user.create({
        data: { id: managerId, orgId, email: `mgr-${managerId}@test.com`, role: 'PROCUREMENT_MANAGER' }
      }).catch(() => {});
      await prisma.user.create({
