...
export default fp(async function verifyRoutes(fastify: FastifyInstance) {
  fastify.get('/v1/public/verify/bid/:id', {
    schema: {
      params: { type: 'object', properties: { id: { type: 'string', format: 'cuid' } }, required: ['id'] },
      response: { 200: { type: 'object', properties: { commitment: { type: 'string' }, plaintextBid: { type: 'object' }, salt: { type: 'string' }, isValid: { type: 'boolean' }, proof: { type: 'string' } }, required: ['commitment', 'plaintextBid', 'salt', 'isValid', 'proof'] } },
    },
