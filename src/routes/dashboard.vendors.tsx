...
export const Route = createFileRoute("/dashboard/vendors")({
  head: () => ({
    meta: [
      { title: "Vendors — SealedBid" },
      { name: "description", content: "Browse and manage registered vendors, KYC status and bidding activity." },
    ],
  }),
      ...
      <div className="mx-auto max-w-[1280px] px-6 py-10">
        ...
        <div className="grid gap-px overflow-hidden rounded-xl border border-border bg-border md:grid-cols-4">
          {([
            ...
            { k: "KYC verified", v: String(verified), sub: `${pending} pending · ${expired} expired` },
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <div className="flex gap-1.5">
            {(["All", "Verified", "Pending", "Expired"] as const).map((f) => {
              const count = f === "All" ? vendors.length : vendors.filter((v) => v.kyc === f).length;
              return (
                <button
                  key={f}
          ...
          <div className="rounded-xl border border-border bg-graphite p-5 text-ivory dark:bg-surface">
            ...
            <ol className="mt-4 space-y-3 text-[13px]">
              {[
                ...
                "SealedBid generates cryptographic binding token.",
