    ...
    <div className="space-y-6">
      ...
      <div className="mt-8 glass-card relative rounded-xl shadow-[0_30px_80px_-30px_rgba(0,0,0,0.25)] hover:translate-y-0">
        ...
        <ol className="divide-y divide-border">
          {items.map((it) => (
            <li key={it.name} className="flex items-center justify-between px-5 py-4 group hover:bg-surface/50 transition-colors">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-[14px] font-medium text-foreground/90 group-hover:text-primary transition-colors">{it.name}</div>
                  <div className="font-mono text-[10.5px] text-muted-foreground mt-1">{it.size} · {it.type}</div>
                </div>
              </div>
